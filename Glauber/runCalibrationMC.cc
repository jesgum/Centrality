...
void runCalibrationMC(TString estimator = "FT0M" , TString histogramName = "hFT0M_Collisions"){
  gStyle->SetOptStat(0);
  // const char* subwagon = "";
  // const char* subwagon = "_rejectpileup";
  // const char* subwagon = "_vtxZ";
  // const char* subwagon = "_rejectnobc";
  // const char* subwagon = "_rejectnobc_rejectpileup";
  const char* subwagon = "_rejectnobc_vtxZ";
  ...
  TString filenameData = "../AnalysisResults/LHC25ae_pass2_extra2/AR_564374.root";
  TString filenameMC = "../AnalysisResults/LHC25h3c_extra2/AR_564374.root";
  ...
  for (int ii = 0; ii < nRuns; ii++) {
    ...
    if(lFitRangeMax<10000) lFitRangeMin = 0.02*GetBoundaryForPercentile(hV0Mfine, 0.01);
  ...
  TFile *mcCalibFileDefault = new TFile(Form("mc/%s_mcCalibDefault.root",estimator.Data()), "RECREATE");
  //return pow(((pars[0] + pars[1] * pow(x, pars[2])) - pars[3]) / pars[4], 1.0f / pars[5]);
  ...
  c5->SaveAs(Form("%s_monteCarloFitTrend.pdf", estimator.Data()));
  c5->SaveAs(Form("%s_monteCarloFitTrend.root", estimator.Data()));
}