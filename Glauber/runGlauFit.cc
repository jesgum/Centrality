...
TH1F* RebinAboveThreshold(TH1F *histo, float threshold, int rebinfactor = 2){
  // function to rebin histogram only above a certain threshold.
  // necessary to keep fine granularity at low multiplicity
  Float_t boundaries[10000];
  Int_t nbins = 0;
  
  Int_t ii=1;
  
  while(ii<histo->GetNbinsX()+1){
  //for(Int_t ii=1; ii<histo->GetNbinsX()+1; ii++){
    boundaries[nbins++] = histo->GetBinLowEdge(ii);
    if(histo->GetBinCenter(ii)>threshold) ii+= rebinfactor;
    ii++;
    //if(ii > histo->GetNbinsX() ) ii = histo->GetNbinsX();//
  }
  TH1F *histoNew = new TH1F(Form("%s_rebinned", histo->GetName()), "", nbins-1, boundaries);
  for(Int_t ii=1; ii<histoNew->GetNbinsX()+1; ii++){
    int bin1 = histo->FindBin(histoNew->GetBinLowEdge(ii)+1e-6);
    int bin2 = histo->FindBin(histoNew->GetBinLowEdge(ii+1)-1e-6);
    Double_t error = 0;
    Double_t integral = histo->IntegralAndError(bin1, bin2, error);
    
    histoNew->SetBinContent(ii, integral);
    histoNew->SetBinError(ii, error);
  }
...
//________________________________________________________________
Double_t EvaluateHisto(TH1 *histo, Double_t x){
 //This function evaluates the value of a histogram at point X
  Int_t lBinNumber = histo->FindBin(x);
  Double_t lBinCenter = histo->GetBinCenter(lBinNumber);
  Double_t lBinWidth = histo->GetBinWidth(lBinNumber);
  Double_t lThisValue = histo->GetBinContent(lBinNumber);
  Double_t lNeighbourValue = histo->GetBinContent(lBinNumber+1);
  if(x < lBinCenter) lNeighbourValue = histo->GetBinContent(lBinNumber-1);
  if(x < lBinWidth/2.0) lNeighbourValue = lThisValue+histo->GetBinContent(lBinNumber)-histo->GetBinContent(lBinNumber+1);
  
  Double_t lFraction = TMath::Abs(x - lBinCenter)/lBinWidth;
  return lThisValue + (lNeighbourValue-lThisValue)*lFraction;
}

//________________________________________________________________
Double_t IntegrateToInfinity(TH1 *histo, Double_t start = 0.0f){
  // integrate histogram content to infinity with some smoothening for wide bins
  int bin = histo->FindBin(start);
  double center = histo->GetBinCenter(bin);
  double width = histo->GetBinWidth(bin); // bin width of this bin
  double integral = 0.0f;
  
  int neighbour = bin + 1;
  if(start < histo->GetBinCenter(bin)) neighbour = bin;
  double neighbourWidth = histo->GetBinWidth(neighbour);
  double binContentProportional = 0.5*(histo->Interpolate(start) + histo->GetBinContent(neighbour));
  
  //double effectiveWidth = 0.5*neighbourWidth + start - (center+0.5*width);
  double effectiveWidth = 0.5*neighbourWidth + center+(width*0.5)-start;
  if(start < histo->GetBinCenter(bin))
    effectiveWidth = center-start;
  
  integral += binContentProportional*effectiveWidth/width;
  
  int integrateFrom = bin+1;
  if(start < histo->GetBinCenter(bin)){
    integral += 0.5 * histo->GetBinContent(bin);
  }else{
    integral += 0.5 * histo->GetBinContent(bin+1);
    integrateFrom++;
  }
  integral += histo->Integral(integrateFrom, histo->GetNbinsX()+1);
  //cout<<"Complement with integral: "<<histo->Integral(integrateFrom, histo->GetNbinsX()+1)<<endl;
  return integral;
}
...
int runGlauFit(TString lInputFileName = "AnalysisResultsLHC24ar.root", Double_t lFitRange = /*350*/500., TString histogramName = "hFT0C_BCs", TString basename = "basehistos")
{
  ...
  // HISTOGRAMS SHOULD BE ACQUIRED BY NOW
  //TH1F *hV0Mfine = (TH1F*) hFT0C_TVX_FV0OrA_CollBC->Clone("hV0Mfine");
  ...
  lFitRange = 0.012*GetBoundaryForPercentile(hV0Mfine, 0.01);
  ...
  // HERE: STOP FOR INSPECTION IF NECESSARY
  ...
  if(!hNpNc){
    cout<<"basehistos.root not found! Stopping!"<<endl; 
    return -1;
  }
