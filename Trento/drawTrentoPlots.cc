...
void drawTrentoPlots(  TString lInputFileName = "trentofit.root",
                       TString lCalibFileName = "trentofit_calib.root") {
  TString lRunNumber = "572183";
  TString lXaxisTitle = "FT0C Amplitude";
  Bool_t lMode = kFALSE;
  TFile *file = new TFile(lInputFileName, "READ");
  file->ls();

  TString calibFileName = "trentocalib.root";
  TFile *file2 = new TFile(lCalibFileName, "READ");
  file2->ls();
...
//  return ;
  ...
  if(!hV0M) {
    cout<<"hData not found"<<endl;
    hV0M = (TH1F*) file2->Get("hV0MUltraFine");
  }
...
//  TH1F *hRawStitched = (TH1F*) file2->Get("hCalib");
  ...
  if (!hGlauberFunctionHyperFine) {
    hGlauberFunctionHyperFine = (TH1F*) file2->Get("hGlauberFine");
  }
  ...
  hV0M->GetYaxis()->SetRangeUser(0.25,hV0M->GetMaximum()*20);
  ...
  hV0M->GetXaxis()->SetTitle("FT0C Amplitude");
  ...
  legO->AddEntry(hGlauberFunctionHyperFine, "Trento + NBD fit");
  legO->Draw();
...
//  hRatioWide->SetLineWidth(2);
  ...
  TString exportName = lCalibFileName; 
